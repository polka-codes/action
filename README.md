...
These variables apply to issue, task, and review automation. Remote-runner mode uses GitHub Actions OIDC and the dispatched session token instead; it does not require model API credentials or an extra GitHub token secret.
...
Note: Provide `runner_payload` for remote-runner mode, or at least one of `issue_number`, `pr_number`, or `task` for issue/task/review automation. Do not combine these modes. `cli_version` only controls the CLI used by issue/task/review automation; remote-runner mode always installs `@polka-codes/runner@latest`.
...
For issue, task, and review automation, configure the following environment variables in your repository's secrets:
...
Create `.github/workflows/polka-codes-runner.yml` with this standalone workflow:

```yaml
name: Polka Codes Remote Runner
run-name: Polka Codes Runner - ${{ github.event.client_payload.taskId }}

on:
  repository_dispatch:
    types: [remote-runner-session]
...
jobs:
  run:
    ...
    steps:
      ...
      - name: Run Polka Codes Runner
        uses: polka-codes/action@master
        with:
          runner_payload: ${{ toJson(github.event.client_payload) }}
```

The workflow checks out the dispatched revision once. The Action validates the nonempty `taskId`, `sessionToken`, and `apiUrl` strings, installs `@polka-codes/runner@latest`, and passes those three values to the runner. The dispatched `apiUrl` selects the service, including staging. Other dispatch fields are left to their owners.

The runner acquires its own GitHub Actions OIDC credentials using `id-token: write`; no token-fetch step or model API credentials are needed. Both GitHub-hosted and self-hosted Actions jobs are supported. The Action streams runner output, waits for completion, and fails the job if the runner cannot start or exits unsuccessfully.
