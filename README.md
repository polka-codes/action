...
### Remote runner callback and execution budget

Remote dispatch payloads must include `taskId`, `sessionToken`, and `apiUrl`. The callback must be the HTTPS origin `https://polka.codes` or `https://staging.polka.codes`; credentials, paths, query strings, and other origins are rejected before checkout and OIDC access. If `runner_api_url` is supplied, it must agree with the payload. The Action passes the callback explicitly and fails on a nonzero runner exit. A requested ref is fetched and its exact `FETCH_HEAD` is checked out, so an existing local branch cannot leave execution on a stale commit.

An optional positive integer `timeout` in the dispatch payload is an execution budget in seconds. On the supported Ubuntu runner, GNU `timeout` terminates the runner process group when that budget expires. Without it, the configured GitHub job timeout applies; there is no five-minute Action cutoff. Checkout and installation remain subject to the job timeout. The repository workflow must include the task ID in `run-name` so the application can reconcile the exact job.

The app requires structured command results from the repaired runner package. Release that package and update the installed/pinned runner version together with this Action bundle; an old runner emitting text/media result arrays is incompatible.
